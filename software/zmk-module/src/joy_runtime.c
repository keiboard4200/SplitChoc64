...
static const struct trig_q10 trig_table[] = {
    {-45,  724,  -724}, {-40,  784,  -658}, {-35,  839,  -587},
    {-30,  887,  -512}, {-25,  928,  -433}, {-20,  962,  -350},
    {-15,  989,  -265}, {-10, 1008,  -178}, { -5, 1020,   -89},
    {  0, 1024,     0}, {  5, 1020,    89}, { 10, 1008,   178},
    { 15,  989,   265}, { 20,  962,   350}, { 25,  928,   433},
    { 30,  887,   512}, { 35,  839,   587}, { 40,  784,   658},
    { 45,  724,   724},
};
...
static int joy_settings_set(const char *name, size_t len, settings_read_cb read_cb, void *cb_arg) {
    ...
    /* Start from known values so a malformed/short backend read can never
     * leave stack bytes interpreted as joystick configuration. */
    struct joy_runtime_state loaded = {0};
    int rc = read_cb(cb_arg, &loaded, sizeof(loaded));
    if (rc < 0) return rc;
    if (rc != sizeof(loaded)) return -EINVAL;
...
struct joy_behavior_config { uint8_t command; };
...
static int joy_behavior_pressed(struct zmk_behavior_binding *binding,
    ...
    switch (cfg->command) {
    case JOY_CMD_TOGGLE: joy_state.enabled = !joy_state.enabled; break;
    case JOY_CMD_ANGLE_INC: joy_state.angle_deg = MIN((int)joy_state.angle_deg + JOY_ANGLE_STEP, JOY_ANGLE_MAX); break;
    case JOY_CMD_ANGLE_DEC: joy_state.angle_deg = MAX((int)joy_state.angle_deg - JOY_ANGLE_STEP, JOY_ANGLE_MIN); break;
    case JOY_CMD_SPEED_INC: joy_state.speed_pct = MIN((int)joy_state.speed_pct + JOY_SPEED_STEP, JOY_SPEED_MAX); break;
    case JOY_CMD_SPEED_DEC: joy_state.speed_pct = MAX((int)joy_state.speed_pct - JOY_SPEED_STEP, JOY_SPEED_MIN); break;
    case JOY_CMD_RESET: joy_reset_defaults(); break;
    case JOY_CMD_DEADZONE_INC: joy_state.deadzone_pct = MIN((int)joy_state.deadzone_pct + JOY_DEADZONE_STEP, JOY_DEADZONE_MAX); break;
    case JOY_CMD_DEADZONE_DEC: joy_state.deadzone_pct = MAX((int)joy_state.deadzone_pct - JOY_DEADZONE_STEP, JOY_DEADZONE_MIN); break;
    case JOY_CMD_MIN_SPEED_INC: joy_state.min_speed_pct = MIN((int)joy_state.min_speed_pct + JOY_MIN_SPEED_STEP, JOY_MIN_SPEED_MAX); break;
    case JOY_CMD_MIN_SPEED_DEC: joy_state.min_speed_pct = MAX((int)joy_state.min_speed_pct - JOY_MIN_SPEED_STEP, JOY_MIN_SPEED_MIN); break;
    case JOY_CMD_CURVE_INC: joy_state.curve_level = MIN((int)joy_state.curve_level + 1, JOY_CURVE_MAX); break;
    case JOY_CMD_CURVE_DEC: joy_state.curve_level = MAX((int)joy_state.curve_level - 1, JOY_CURVE_MIN); break;
...
static uint16_t joy_curve_q10(uint16_t mag_q10) {
    ...
    case 0: return (uint16_t)m;
    case 1: return (uint16_t)((3u * m + ((m * m) / 1024u)) / 4u);
    case 2: return (uint16_t)((m + ((m * m) / 1024u)) / 2u);
    case 3:
    default: return (uint16_t)((m * m) / 1024u);
    }
}

static void joy_apply_response(int32_t *x, int32_t *y) {
    uint32_t ax = joy_abs_i32(*x), ay = joy_abs_i32(*y);
    /* Saturate the square terms before the 32-bit integer square root. Normal
     * joystick samples are tiny, but this keeps malformed input from wrapping. */
    uint64_t mag_sq = (uint64_t)ax * ax + (uint64_t)ay * ay;
    uint32_t mag = joy_isqrt_u32((uint32_t)MIN(mag_sq, (uint64_t)UINT32_MAX));

    if (mag == 0) { *x = 0; *y = 0; return; }
    ...
    if (mag_q10 <= dead_q10) { *x = 0; *y = 0; return; }
    ...
    *x = (int32_t)(((int64_t)*x * response_q10) / 1024);
    *y = (int32_t)(((int64_t)*y * response_q10) / 1024);
}
...
static void joy_input_handler(struct input_event *evt, void *user_data) {
    ...
    int32_t x = joy_acc_x, y = joy_acc_y;
    joy_acc_x = 0;
    joy_acc_y = 0;
    ...
    /* Screen coordinates have +Y downward, so this matrix makes a positive
     * correction clockwise on screen. Example: 11 o'clock input with +30 deg
     * correction becomes approximately 12 o'clock cursor movement. */
    int32_t rx = (int32_t)(((int64_t)x * t->cos_q10 - (int64_t)y * t->sin_q10) / 1024);
    int32_t ry = (int32_t)(((int64_t)x * t->sin_q10 + (int64_t)y * t->cos_q10) / 1024);

    rx = (int32_t)(((int64_t)rx * joy_state.speed_pct) / 100);
    ry = (int32_t)(((int64_t)ry * joy_state.speed_pct) / 100);
...
INPUT_CALLBACK_DEFINE(DEVICE_DT_GET(DT_CHOSEN(splitchoc64_joy_input)), joy_input_handler, NULL);
